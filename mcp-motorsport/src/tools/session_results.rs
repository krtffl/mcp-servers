//! `get_session_results` tool — fetch results for any session type.
//!
//! Works for FP1, FP2, FP3, Qualifying, Sprint, and Race sessions.
//! Resolves the session key, fetches positions and driver info,
//! then returns a results table sorted by final position.
...
/// Fetch session results for a given Grand Prix and session type.
///
/// # Errors
///
...
pub async fn execute(
    ...
    let cache_key = format!("session_results:{year}:{grand_prix}:{session}");

    let value = cache
        .get_or_fetch(&cache_key, || async {
            let session_key = resolve_session_key(year, grand_prix, session, http, cache, config)
                .await
            ...
            // Keep the last position entry per driver (final classification).
